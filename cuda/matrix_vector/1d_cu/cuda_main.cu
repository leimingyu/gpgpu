		...
		float* __restrict__ C)
{
	...
	int gx = threadIdx.x + __mul24(blockIdx.x, blockDim.x); // rows
	int lx = threadIdx.x;

	int pos  = gx * cols; 	// row start address
	...
	for(int i=0; i<cols; i+=4)
	{
		...
		int addr = i<<7 + lx; 
		A_sum[addr]       = A[pos+i];
		A_sum[addr + 128] = A[pos+i + 1];
		A_sum[addr + 256] = A[pos+i + 2];
		A_sum[addr + 384] = A[pos+i + 3];
