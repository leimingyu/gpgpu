...
// tile A: 2
//----------------------------------------------------------------------------//
__global__ void kernel_sgemv_1d_512b_2w (const int rows,
		const int cols,
		const int iters,
		...
		float* __restrict__ C)
{
	...
	// 2nd 512
	//int lx1 = lx + 512;
	int lx1 = lx + blockDim.x;
	float c1 = 0.f;
	if(lx1 < cols) {
		c1 = A[bx * cols + lx1] * B[lx1];
	}
...
void test_v1a(int rows, int cols)
{
	...
	//--------------------------------------------------------------------------
    dim3 Blk_config = dim3(512, 1, 1);                                           
    dim3 Grd_config = dim3(rows, 1, 1);

	//printf("iters: %d\n", BLK(cols, 4));

	kernel_sgemv_1d_512b_2w <<< Grd_config, Blk_config>>>(rows, 
			cols, 
			BLK(cols, 512),
			d_A,
			d_B,
