		...
		float* __restrict__ C)
{
	...
	for(int i=0; i<iters; i++)
	{
		...
		if(bid < cols) {
			tmp  += A[offset] * sdata[threadIdx.x];
			tmp1 += A[offset +  stride1] * sdata[threadIdx.x];
			tmp2 += A[offset +  stride2] * sdata[threadIdx.x];
			tmp3 += A[offset +  stride3] * sdata[threadIdx.x];
		}
	}
...
void test_v1a(int rows, int cols)
{
    ...
    dim3 Grd_config = dim3(BLK(rows,512), 1, 1);
