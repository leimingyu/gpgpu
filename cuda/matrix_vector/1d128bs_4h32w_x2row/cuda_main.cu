		...
		float* __restrict__ C)
{
	...
	// warp 2 = row 4 5 
	// warp 3 = row 6 7 
	int row_id = bx_startrow + (warp_id<<1);
	int row_id1 = row_id + 1; 
	...
	for (int i=0; i<col_iters; i++)
	{
		...
		if(row_id1 < rows)
		{
			// work on 1st 32 threads/cols
			if(col_idx < cols) {
				tmp1  += A[row_idx1  + col_idx] * B_sm[loc_c];
			}

			// work on 2nd 32 threads/cols
			if(col_idx1 < cols) {
				tmp1 += A[row_idx1 + col_idx1] * B_sm[loc_c1];
			}

			// work on 3rd 32 threads/cols
			if(col_idx2 < cols) {
				tmp1 += A[row_idx1 + col_idx2] * B_sm[loc_c2];
			}

			// work on 4th 32 threads/cols
			if(col_idx3 < cols) {
				tmp1 += A[row_idx1 + col_idx3] * B_sm[loc_c3];
			}
		}
	}
